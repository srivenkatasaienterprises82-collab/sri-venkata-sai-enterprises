...
export default defineConfig({
  // ... other config
  defaultMachine: "large-1x", // Default machine for all products
...
# Sri Venkata Sai Enterprises — Dev Log

This file is preserved across sessions. Update it when starting/finishing major work so it can report "what did we do so far?"

## Goal
- Seed static product/brand/category data into Sanity and wire the site to read from Sanity first, with static fallback; automate price syncing from Amazon/Flipkart
...
- Removed all `coalesce()` calls from `src/sanity/queries.ts` — image fields use raw field name directly (`coverImage`, `logo`, `image`, `heroSlides`, etc.) because `defineLive().sanityFetch()` in next-sanity v13.1.1 doesn't properly return coalesce results
- `images[]` and `heroSlides[]` projections also reverted to raw field names
- TypeScript compiles with zero errors (`npx tsc --noEmit`)
- Added Vercel Deploy Hook trigger step to `price-sync.yml` and `launch-check.yml`
- Created Vercel Deploy Hook (`price-sync`, main branch) and added `VERCEL_DEPLOY_HOOK_URL` to GitHub Secrets
- Pushed all changes to GitHub; Vercel redeploys triggered
- Removed 8 brands (AI+, CMF, Coolpad, Itel, Jio, Lava, Peace, Snexian) from brand carousel: deleted from `src/lib/data/brands.ts` and set `hidden: true` on their Sanity documents via API mutation
- **Identified root cause of null prices**: No product in `src/lib/data/products.ts` has a top-level `price` field — all prices are only inside `variants[].price`. The seed script's `productToSanityDoc` reads `product.price` which is always `undefined`, so `JSON.stringify` drops the key and Sanity documents never get a `price` field. The price sync automation correctly skips all products (no `amazonUrl`/`flipkartUrl` set), so it did NOT cause the null prices. Prices have been null in Sanity since the first seed.

### Done (July 2)
- Fixed seed script `productToSanityDoc`: falls back to `product.variants[0]?.price` when `product.price` is undefined
- Added `amazonUrl` and `flipkartUrl` to `Product` interface in `src/lib/data/products.ts`
- Seed script now includes `amazonUrl`/`flipkartUrl` in Sanity product docs
- Re-ran seed script: 132/134 products now have prices in Sanity (only AirPods 4 & Infinix Gaming Kit correctly show priceOnEnquiry)
- Triggered Vercel redeploy — site shows all prices correctly

### Done (July 5)
- Extended seed script with 8 new content types: siteSettings, homePage (with featuredCarousel, topRatedPerformance, bentoDeals references), banners (4), testimonials (6), FAQs (8), gallery (3), offers (5, with bento deals linked to homePage), pages (4 — About, Privacy, Terms, Shipping)
- Added `PAGE_QUERY` to `src/sanity/queries.ts` for fetching page documents by slug
- Added `SanityPage` type to `src/sanity/types.ts`
- Added `toPageContent` helper to `src/sanity/transform.ts`
- Updated Trust Strip to accept `settings` prop from Sanity (`src/components/sections/trust-strip.tsx`)
- Updated Brand Carousel to accept `brands` prop from Sanity (replaces hardcoded BRAND_LOGOS) (`src/components/sections/brand-carousel.tsx`)
- Updated homepage to pass `settings` to TrustStrip and `brands` to BrandCarousel (`src/app/page.tsx`)
- Wired Privacy, Terms, and Shipping Policy pages to fetch from Sanity with full static fallback (uses `generateMetadata` for dynamic SEO, `PortableText` from `@portabletext/react` for content rendering)
- Wired About page to use Sanity page title/metadata with portable text for "Our Story" section
...
- Built full TDD automation so newly-launched phones per brand become live Sanity product docs (`enabled: true`, auto-published):
  ...
  - Tests: `tests/test_sanity_api.py`, `test_listing.py` (+fixtures), `test_details.py` (+fixtures), `test_launch_checker.py`. `tests/conftest.py` adds `automation/` to sys.path for bare-import style.
  - `pytest` 9.0.3 local: **16 passed**. 7 commits pushed to `origin/main`.

## Key Decisions
- Removed `coalesce()` from GROQ queries entirely since all image fields are plain URL strings — avoids next-sanity v13.1.1 `filterResponse: false` issue
- Set `hidden: true` on unwanted brands in Sanity rather than deleting them (preserves product references)
...
- `_updatedAt` timestamps on all products are from `createOrReplace` mutations in the seed script
- Vercel Deploy Hook URL: `https://api.vercel.com/v1/integrations/deploy/prj_9fQUzD9H0WSAdHk7WmkhabDak9o3/DbSmNGYZjE`
- GitHub Secret `VERCEL_DEPLOY_HOOK_URL` set in repo

## Relevant Files
- `src/sanity/queries.ts`: All GROQ queries — coalesce removed, raw field names used
- `src/lib/data/brands.ts`: Static brand data — 8 unwanted brands removed (now 14 visible)
- `src/lib/data/products.ts`: Static product data — 134 products with prices only in `variants[]`
- `scripts/seed-sanity.ts`: Seed script — `productToSanityDoc()` needs fallback to first variant's price
- `automation/launch_checker.py`: Price sync script — skips products without URLs (correct behavior)
